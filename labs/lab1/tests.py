...
def test_check_if_first_letter_of_string_is_uppercase(results_dict):
        """
        Checks that the first letter reurned in test_check_if_first_letter_of_string_is_uppercase() is Uppercase
        """
        print("")
        print("Testing lab1.test_check_if_first_letter_of_string_is_uppercase()")
        message = "----"
        try:
            result = lab1.check_if_first_letter_of_string_is_uppercase()
            if not isinstance(result, str):
                message += 'Did not return a string - Fail'
                test_failed(results_dict)
            else:
                if result[0].isupper():
                    message += 'Returned a uppercase as fist letter of string - Pass'
                    test_passed(results_dict)
                else:
                    message += 'Did not return an uppercase as first letter - Fail'
                    test_failed(results_dict)
        except AttributeError:
            message += 'test_check_if_first_letter_of_string_is_uppercase function - Fail'
            test_failed(results_dict)
        print(message)

def test_string_count_is_5(results_dict):
        """
        Checks that the returned string count in test_word_count_is_5() is 5
        """
        print("")
        print("Testing lab1.test_string_count_is_5()")
        message = "----"
        try:
            result = lab1.string_count_is_5()
            count = 0
            if not isinstance(result, str):
                message += 'Did not return a string - Fail'
                test_failed(results_dict)
                ...
                for char in result:
                    count+=1
                if count is not 5:
                    message += 'Count not equals to five - fail'
                    test_failed(results_dict)
                else:
                    message += 'Count equals five - pass'
                    test_passed(results_dict)
        except AttributeError:
            message += 'test_check_if_first_letter_of_string_is_uppercase function - Fail'
            test_failed(results_dict)
...
def run():
    ...
    test_check_if_first_letter_of_string_is_uppercase(results_dict)
    test_string_count_is_5(results_dict)
